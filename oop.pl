...
% DEF_CLASS/3:
% def_class/3 è l'implementazione completa
def_class(ClassName, Parents, Parts) :-
    atomic(ClassName),
    is_list(Parents),
    is_list(Parts),
    \+(is_class(ClassName)),
    % non può essere già una classe
...
% CONCATENATE_PARTS/3:
% per comodità quando definisco la classe
% implemento già i metodi delle superclassi
concatenate_parts([], _, []).
...
% figlie (un float non può essere inserito
% in un intero)
...
% NOT_PART_MEMBER/2:
% controllo se part esiste già nelle superclassi
not_part_member(_, []).
...
% controlli su parts
part_is_correct([]).
part_is_correct([X | Rest]) :- % field di arietà 2
    compound(X),
    functor(X, field, 2),
    arg(1, X, FieldName),
    atom(FieldName),
    part_is_correct(Rest).
part_is_correct([X | Rest]) :- % field di arietà 3
    compound(X),
    functor(X, field, 3),
    arg(1, X, FieldName),
    atom(FieldName),
    ...
    check_type(A, B),
    part_is_correct(Rest).
part_is_correct([X | Rest]) :- % method di arietà 3
    compound(X),
    functor(X, method, 3),
    arg(1, X, MethodName),
    atom(MethodName),
...
% MAKE/3:
% caso di debug 1, InstanceName è una variabile
% NON istanziata
make(InstanceName, ClassName, Field) :-
    is_class(ClassName),
    % fa anche il controllo atomic(ClassName)
...
verifyCorrispondency([field(Name, _) | Parts],
                     ...
                     Name, Valore) :-
    % faccio passare tutta la lista, se Name è presente in
...
% per semplicità ci sono i due casi, quelli
